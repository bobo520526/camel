...
namespace Common\Library\weixinPay\lib;
...
class JsApiPay {
    ...
    public function GetOpenid() {
        ...
        if (!isset($_GET['code'])) {
            ...
            $baseUrl = urlencode(SITE_URL . $_SERVER['PHP_SELF'] . $_SERVER['QUERY_STRING']);
            $url = $this->__CreateOauthUrlForCode($baseUrl);
            Header("Location: $url");
            file_put_contents('open1.txt', $_SERVER['PHP_SELF'] . $_SERVER['QUERY_STRING']);
            exit();
        } else {
            //获取code码，以获取openid
            $code = $_GET['code'];
            $openid = $this->getOpenidFromMp($code);
            file_put_contents('open.txt', $openid);
            return $openid;
        }
    }
    ...
    private function __CreateOauthUrlForCode($redirectUrl) {
        ...
        //暂时注释  重定向TODO
        //$urlObj["redirect_uri"] = "http://skiing.8848.com";
     ...
     * 测试
     */
    public function changeRedirectUrl() {
        $appid = WxPayConfig::$appid;
        $scope = 'snsapi_base';
        $state = '';
        $code = '';
