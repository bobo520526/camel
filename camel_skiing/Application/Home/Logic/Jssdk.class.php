...
// 随机字符串
  private function createNonceStr($length = 16) {
    ...
    public function get_access_token(){
        //判断是否过了缓存期
        $wechat = M('wechat_config')->find();
        $expire_time = $wechat['access_token_time'];
        if($expire_time > time()){
        ...
        M('wechat_config')->where(array('id'=>$wechat['id']))->save(array('access_token'=>$return['access_token'],'web_expires'=>$web_expires));
        return $return['access_token'];
    }   
